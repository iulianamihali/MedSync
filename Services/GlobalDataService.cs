    ...
    public class GlobalDataService : IGlobalDataService
    {
        ...
        public async Task<List<SpecialtyDto>> GetSpecialties()
        {
            var specialties = await _context
                .Specialties.Select(x => new SpecialtyDto(x))
                .ToListAsync();
            return specialties;
        }
        ...
        public async Task<List<ServiceSelectDto>> GetServices()
        {
            var services = await _context
                .Services.Select(x => new ServiceSelectDto { Id = x.Id, Name = x.Name })
                .ToListAsync();
            return services;
        }
        ...
        {
            var institutionId = await _context
                .Institutions.Where(c => c.Code == codeInstitution)
                .Select(x => x.Id)
                .FirstOrDefaultAsync();
            var result = await _context
                .InstitutionServices.Where(i => i.InstitutionId == institutionId)
                .Select(x => new SpecialtyDto { Id = x.Specialty.Id, Name = x.Specialty.Name })
                .Distinct()
                .ToListAsync();
            return result;
        }
        ...
        public async Task<List<ClinicLocationResponseDto>> GetClinicLocations()
        {
            var response = await _context
                .Institutions.Select(x => new ClinicLocationResponseDto
                {
                    Id = x.Id,
                    InstitutionName = x.Name,
                    InstitutionSpecialties = x
                        .InstitutionServices.Select(s => s.Specialty.Name)
                        .ToList(),
                    FullAddress =
                        $"{x.Address.Country} {x.Address.Number}, {x.Address.PostalCode} {x.Address.City}, {x.Address.Country}",
