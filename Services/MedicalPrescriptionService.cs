...
namespace MedSync.Services
{
    public class MedicalPrescriptionService : IMedicalPrescriptionService
    {
        ...
        public async Task<Guid> AddMedicalPrescriptionAsync(
        ...
        public async Task<MedicalPrescriptionPdfDto> GetMedicalPrescriptionPdfDataAsync(
            Guid prescriptionId
        )
        {
            var medicalPrescription = await _context
                .Prescriptions.Where(p => p.Id == prescriptionId)
                .Select(x => new MedicalPrescriptionPdfDto
                {
                    IssuedAt = x.Appointment.CreatedAt,
                    InstitutionName = x.Appointment.Institution.Name,
                    InstitutionAddress =
                        x.Appointment.Institution.Address != null
                            ...
                            : "-",
                    PatientFirstName =
                        x.Appointment.Patient != null
                            ? x.Appointment.Patient.User.FirstName
                            : x.Appointment.UnregisteredPatient.FirstName,
                    PatientLastName =
                        x.Appointment.Patient != null
                            ? x.Appointment.Patient.User.LastName
                            : x.Appointment.UnregisteredPatient.LastName,
                    PatientCnp =
                        x.Appointment.Patient != null
                            ? x.Appointment.Patient.Cnp
                            : x.Appointment.UnregisteredPatient.Cnp,
                    PatientDateOfBirth =
                        x.Appointment.Patient != null
                            ? x.Appointment.Patient.User.DateOfBirth
                            : null,
                    Diagnosis = x.Diagnosis,
                    DoctorFullName =
                        $"{x.Appointment.Doctor.User.FirstName} {x.Appointment.Doctor.User.LastName}",
                    Medications = x
        ...
        public async Task<
            List<GetAppointmentPrescriptionsResponseDto>
        > GetAppointmentPrescriptionsAsync(Guid appointmentId)
        {
