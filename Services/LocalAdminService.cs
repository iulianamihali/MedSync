...
namespace MedSync.Services
{
    public class LocalAdminService : ILocalAdminService
    {
        ...
        public async Task<CountsStatCardsResponseDto> GetDashboardStatCardsAsync(
        ...
        )
        {
            ...
            var appointmentsCount = await _context
                ...
                .CountAsync();
            var totalDoctors = await _context
                .InstitutionUsers.Include(u => u.User)
                .Where(u =>
                ...
                .CountAsync();
            var canceledAppointments = await _context
                .Appointments.Where(a =>
                    a.InstitutionId == request.InstitutionId
                    && (a.StartDateTime >= fromLocal && a.StartDateTime < toLocal.AddDays(1))
                ...
                .CountAsync();
            var appointmentsWithReferral = await _context
                .Appointments.Where(a =>
                    a.InstitutionId == request.InstitutionId
                    && (a.StartDateTime >= fromLocal && a.StartDateTime < toLocal.AddDays(1))
                    && a.ReferralCode != null
        ...
        public async Task<List<RecentAppointmentsDto>> GetDetailsRecentAppointmentsAsync(
            Guid institutionId
        )
        {
            var response = await _context
                .Appointments.Include(u => u.Patient)
                ...
                .Where(u =>
                    ...
                    && (
                        ...
                        || u.Status == AppointmentStatusEnumType.Rescheduled
                    )
                )
                .Select(u => new RecentAppointmentsDto
                {
                    AppointmentId = u.Id,
        ...
        public async Task<List<DoctorReqPopUpResponseDto>> GetDoctorRequestDetailsAsync(
            Guid institutionId
        )
        {
            var result = await _context
                .DoctorRequests.AsNoTracking()
                ...
                .Select(i => new DoctorReqPopUpResponseDto
                {
                    ...
                    Specialization = string.Join(
                        ", ",
                        i.User.Doctor.DoctorSpecialties.Select(x =>
                                x.InstitutionService.Specialty.Name
                            )
                            .ToList()
                    ),
        ...
        {
            var result = await _context
                .DoctorRequests.Include(u => u.User)
